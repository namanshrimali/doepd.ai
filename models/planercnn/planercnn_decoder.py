...
def detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, gt_parameters, config):
    ...
    if positive_roi_bool.sum() > 0:
        ...
        crop_resize_fn = CropAndResizeFunction
        if config.NUM_PARAMETER_CHANNELS > 0:
            crop_resize_fn = crop_resize_fn.apply(roi_masks[:, :, :, 0].contiguous().unsqueeze(1), boxes, box_ids, config.MASK_SHAPE[0], config.MASK_SHAPE[1], 0)
            masks = Variable(crop_resize_fn.data, requires_grad=False).squeeze(1)
            masks = torch.round(masks)
            crop_resize_fn = crop_resize_fn.apply(roi_masks[:, :, :, 1].contiguous().unsqueeze(1), boxes, box_ids, config.MASK_SHAPE[0], config.MASK_SHAPE[1], 0)
            parameters = Variable(crop_resize_fn.data, requires_grad=False).squeeze(1)
            masks = torch.stack([masks, parameters], dim=-1)
        else:
            crop_resize_fn = crop_resize_fn.apply(roi_masks.unsqueeze(1), boxes, box_ids, config.MASK_SHAPE[0], config.MASK_SHAPE[1], 0)
            masks = Variable(crop_resize_fn.data, requires_grad=False).squeeze(1)            
            masks = torch.round(masks)            
            pass
        ...
        if mode == 'inference':
            ...
            if len(detections) > 0:
                ...
                if len(detections) > 0:
                    ...
                    if self.config.GPU_COUNT:
                        box_ids = box_ids.cuda()
                    crop_resize_fn = CropAndResizeFunction.apply(roi_gt_masks.unsqueeze(1), boxes, box_ids, self.config.FINAL_MASK_SHAPE[0], self.config.FINAL_MASK_SHAPE[1], 0)
